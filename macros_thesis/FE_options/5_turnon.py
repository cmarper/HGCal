...
for name in df_tau:
  ...
  eff = df['efficiency_40']
  eff_smooth = df['efficiency_smooth_40']
  axs[0].plot(df.gentau_vis_pt, eff_smooth, label=legends[name], linewidth=2, color=colors[name])

  ratio = efficiencies_vs_pt[name]['efficiency_smooth_40']/efficiencies_vs_pt[0]['efficiency_smooth_40']
  axs[1].plot(df['gentau_vis_pt'], ratio, color=colors[name], label=legends[name],lw=2)

axs[0].legend(loc = 'lower right', fontsize=14, title='$E^{L1,tau}_{T} > 40 GeV$')
axs[0].set_xlim(20., 92.)
axs[0].set_ylim(0., 1.1)
axs[0].set_ylabel('Efficiency')
axs[0].grid()
#axs[0].set_xlabel(r'$RMS(p_{T}^{L1}/p_{T}^{gen})$')
axs[1].set_xlim(20., 92.)
axs[1].set_ylim(0.75,1.35)
axs[1].set_xlabel(r'$p^{gen,\tau}_{T}\,[GeV]$')
axs[1].set_ylabel('Ratio to Threshold')
axs[1].grid()
plt.savefig(plotdir+'feoptions_eff_ratio.png')
plt.savefig(plotdir+'feoptions_eff_ratio.pdf')
